...
# all the sources
#
SRCS =	main.c sim-fast.c sim-safe.c sim-cache.c sim-profile.c sim-sid.c \
...
PROGS = sim-fast$(EEXT) sim-safe$(EEXT) sim-eio$(EEXT) \
	sim-bpred$(EEXT) sim-profile$(EEXT) sim-sid$(EEXT)\
...
sim-sid$(EEXT):	sysprobe$(EEXT) sim-sid.$(OEXT) cache.$(OEXT) $(OBJS) libexo/libexo.$(LEXT)
	$(CC) -o sim-sid$(EEXT) $(CFLAGS) sim-sid.$(OEXT) cache.$(OEXT) $(OBJS) libexo/libexo.$(LEXT) $(MLIBS)
...
sim-tests sim-tests-nt: sysprobe$(EEXT) $(PROGS)
	...
	cd tests $(CS) \
	$(MAKE) "MAKE=$(MAKE)" "RM=$(RM)" "ENDIAN=$(ENDIAN)" tests \
		"DIFF=$(DIFF)" "SIM_DIR=.." "SIM_BIN=sim-sid$(EEXT)" \
		"X=$(X)" "CS=$(CS)" $(CS) \
	cd ..
	#cd tests $(CS) \
	#$(MAKE) "MAKE=$(MAKE)" "RM=$(RM)" "ENDIAN=$(ENDIAN)" tests \
...
sim-sid.$(OEXT): host.h misc.h machine.h machine.def regs.h memory.h
sim-sid.$(OEXT): options.h stats.h eval.h cache.h loader.h syscall.h
sim-sid.$(OEXT): dlite.h sim.h
